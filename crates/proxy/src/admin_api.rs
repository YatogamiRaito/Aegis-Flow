...
pub fn create_router(state: ConfigState) -> Router {
    Router::new()
        ...
        .route("/config/servers/{id}", get(get_server).put(update_server).delete(delete_server))
...
mod tests {
    ...
    async fn test_list_servers_empty() {
        ...
        let resp = app.oneshot(
            Request::builder().uri("/config/servers").body(Body::empty()).unwrap()
        ).await.unwrap();
    ...
    async fn test_add_and_get_server() {
        ...
        // Add
        let add_resp = create_router(state.clone()).oneshot(
            Request::builder()
                .method(Method::POST)
                .uri("/config/servers")
        ...
        // Get
        let get_resp = app.oneshot(
            Request::builder().uri("/config/servers/web").body(Body::empty()).unwrap()
        ).await.unwrap();
    ...
    async fn test_version_increments() {
        ...
        create_router(state.clone()).oneshot(
            Request::builder()
                .method(Method::POST)
                .uri("/config/servers")
                .header("content-type", "application/json")
