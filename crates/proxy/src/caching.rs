...
impl CachingManager {
    pub fn new(config: CacheConfig) -> Self {
        Self {
            config,
            strong_etag_cache: Arc::new(parking_lot::RwLock::new(LruCache::new(
    ...
    pub fn generate_etag(&self, path: &Path, metadata: &std::fs::Metadata) -> Option<String> {
        match self.config.etag_mode {
            ...
            EtagMode::Weak => {
                ...
                Some(format!(
                    "W/\"{x}-{y}\"",
                    x = hex::encode(size.to_be_bytes()),
                    y = hex::encode(mtime.to_be_bytes())
                ))
            }
            EtagMode::Strong => {
...
mod tests {
    ...
    fn test_cache_control_assignment() {
        ...
        assert_eq!(
            manager.get_cache_control(Path::new("app.js")).unwrap(),
            "max-age=31536000"
        );
        assert_eq!(
            manager.get_cache_control(Path::new("style.css")).unwrap(),
            "max-age=86400"
        );
