...
impl BucketManager {
    ...
    pub async fn check_limit(&self, key: &str) -> Option<Duration> {
        ...
        let bucket = buckets.entry(key.to_string()).or_insert_with(|| {
            TokenBucket::new(self.zone.burst, self.zone.rate_per_second)
        });
...
where B: From<String>
{
    ...
    *resp.status_mut() = StatusCode::TOO_MANY_REQUESTS;
    ...
    let secs = if retry_after.as_secs() == 0 { 1 } else { retry_after.as_secs() };
    resp.headers_mut().insert(RETRY_AFTER, hyper::header::HeaderValue::from(secs));
